...
if($manager != "callcenter"){
	$manager = "members";
	$page = "Members Manager > Create New Members";
}else{
	$page = "Call Center Manager > Create New Members";
}
...
if($submit != "" || $order != "") {
	...
	if($date_of_birth == "") { $error_txt .= "Error, you did not pick a date of birth. Please pick your date of birth.<br>\n"; }

	// If no errors test for unique email
	if($error_txt == "") {
		$email_test = check_dup_email($email);
		if($email_test < 0) {
	...
	if($error_txt == "") {
		...
		$query = "INSERT INTO members SET created='$now', status='1', email='$email', username='$email', password='$password', nickname='$nickname', first_name='$first_name', last_name='$last_name', date_of_birth='$date_of_birth', bill_name='$bill_name', bill_address1='$bill_address1', bill_address2='$bill_address2', bill_city='$bill_city', bill_state='$bill_state', bill_zip='$bill_zip', bill_country='$bill_country', bill_phone='$bill_phone', ship_name='$ship_name', ship_address1='$ship_address1', ship_address2='$ship_address2', ship_city='$ship_city', ship_state='$ship_state', ship_zip='$ship_zip', ship_country='$ship_country', ship_phone='$ship_phone'";
		$result = mysql_query($query) or die("Query failed : " . mysql_error());

		send_email_login($email, $first_name, $last_name, $email, $password);

		if($order != ""){
			//Goto place CONSUMER order...
			header("Location: " . $base_url . "admin/members_place_order.php?member_id=" . $member_id);
			exit;
		}else{
			//Goto Index
			switch ($manager){
				case "callcenter":
					header("Location: " . $base_url . "admin/callcenter_admin.php");
					exit;
				default:
...
function genPassword(newPass) {
	document.create_member.password.value = newPass;
}
<?php
...
<tr><td align="left"><font size="2">Welcome to the <?=$page?> page, where you create new members of our website.</font></td></tr>
...
?>
<tr><td align="left">
	<FORM name="create_member" Method="POST" ACTION="<?=$_SERVER["REQUEST_URI"]?>" class="wmsform">
	...
	<fieldset>
		...
		<ol>
			...
			<li>
				...
				<INPUT type="text" id="password" name="password" size="10" maxlength="10" value="<?php echo $password; ?>" tabindex="2" /> <input type="button" id="generate_password" name="generate_password" value="Generate Password" onClick="genPassword('<?php echo $newpass; ?>')">
			</li>
			<li>
			...
			<li>
				<label for="date_of_birth">Date of Birth <em>*</em></label>
	    			<input type="text" id="date_of_birth" name="date_of_birth" class="datepicker" tabindex="6" value="<?=$date_of_birth?>" />
			</li>
		</ol>
	</fieldset>
	<fieldset>
	...
	<fieldset>
		...
		<ol>
			...
			<li class="fm-button">
				<input type="submit" name="order" value="Create Member and Place Order" />
			</li>
			</li>
		</ol>
	</fieldset>
