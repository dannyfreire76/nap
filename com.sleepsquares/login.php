...
function send_email_login($email, $first_name, $last_name, $username, $password) {
	...
	if($email != "") {
		...
		$email_str .= "\nIf you'd like to change your password once you've logged in, please visit ".$base_url."customer/update_customer.php";
	...
	if($error_txt == "") {
        ...
        if(mysql_num_rows($resultPW) > 0) {
            ...
            while($linePW = mysql_fetch_array($resultPW, MYSQL_ASSOC)){
				...
				$error_txt .= "Your login information has been sent to the email you provided.<br />\n";
            }
        }
        else {
	...
	if($error_txt == "") {
        ...
        if(mysql_num_rows($resultPW) > 0) {
            ...
            while($linePW = mysql_fetch_array($resultPW, MYSQL_ASSOC)){
                ...
                send_email_login($email, $first_name, $last_name, $username);

				$error_txt .= "Your username has been sent to the email you provided.<br />\n";
            }
        }
        else {
    		$error_txt .= "Login information not found.";
...
<?php if(!$loginOk):?>
	<body bgcolor="#<?php echo $bgcolor; ?>" onLoad="document.forms['login_actual'].elements['username'].focus();">
<?php else:?>
...
<?php else:?>
	...
	<table border="0" width="500" class="boxContent">
		<tr>
			<td>&nbsp;</td>
		</tr>
		<tr>
					...
					<tr>
						<td align="center" valign="top">Please login using the form below:<br />&nbsp;
							...
							<form id="login_actual" action="<?php echo $current_base.'login.php'; ?>" method="POST">
								...
								<ul class="text_left">
									<li class="disc">
										<a href="javascript:void(0)" onclick="$('#frm_forgot_login').slideToggle(350);">Forgot Your Password?</a>
									</li>
									<li class="disc">
										<a href="javascript:void(0)" onclick="$('#frm_forgot_loginUN').slideToggle(350);">Retrieve Username</a>
									</li>
								</ul>
							</form>
						</td>
	...
	<div id="frm_forgot_login" style="display:none;" class="boxContent">
		<p>Please enter your email address to have your password reset and login credentials emailed to you:</p>
		<p>Please allow up to 30 minutes for the email to reach your inbox.</p>
		<form id="forget_form" action="<?php echo $current_base.'login.php'; ?>" method="POST">
			<table border="0">
				<tr>
				...
				<tr>
					<td align="center" colspan="2">
						<input type="submit" id="submit" name="forgotPW" value="Reset Password" />
					</td>
				</tr>
			</table>
		</form>
		<p><b>NOTE:</b> Clicking on the Retrieve Info button above will reset your password. Once <br />
			 you login, you can click on the Your Profile link at the left side of the page to change it.</p>
	</div>

	<div id="frm_forgot_loginUN" style="display:none;" class="boxContent">
		<p>Please enter your email address to have your username emailed to you:</p>
		<p>Please allow up to 30 minutes for the email to reach your inbox.</p>
		<form id="forget_form" action="<?php echo $current_base.'login.php'; ?>" method="POST">
			<table border="0">
				<tr>
				...
				<tr>
					<td align="center" colspan="2">
						<input type="submit" id="submit" name="forgotUN" value="Retrieve Username" />
					</td>
				</tr>
			</table>
		</form>
