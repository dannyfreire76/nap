...
function send_email_login($email, $first_name, $last_name, $username, $password) {
	...
	if($email != "") {
		...
		$subject = $website_title . " Login Info";
		$email_subj = $subject;
...
?>
<form id="c_forget_uandp" action="<?php echo $current_base.'c_forget_uandp.php'.(  $_REQUEST["unOnly"]? '?unOnly=1':''  ); ?>" method="POST">
		...
		<tr>
			<td align="center" colspan="2">
				<?php
					if ( $_REQUEST["unOnly"] ) {
				?>
						<input type="submit" id="submit" name="submit" value="Retrieve Username" />
				<?php
					} else {
				?>
