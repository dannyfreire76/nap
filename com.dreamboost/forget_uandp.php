...
if($submit != "") {
	...
	if($error_txt == "") {
		$queryPW = "SELECT username, contact_name FROM retailer WHERE email='$email'";
        ...
        if (mysql_num_rows($resultPW)>0) {
            while ($linePW = mysql_fetch_array($resultPW, MYSQL_ASSOC)) {
                ...
                $contact_name = $linePW["contact_name"];

                // Generate New Password
                $pas = new password();
                $newpass = $pas->generate();
                $query = "UPDATE retailer SET password='".md5($newpass)."'";
                $query .= "WHERE email='$email'";
                $result = mysql_query($query) or die("Query failed : " . mysql_error());

        		send_email_login($email, $contact_name, $username, $newpass);
            }
        }
        else {
...
?>
<form id="forget_uandp" action="<?php echo $current_base.'forget_uandp.php'; ?>" method="POST">
