<?php
// BME WMS
// Page: Members Manager Edit Member page
// Path/File: /admin/members_admin2_edit.php
// Version: 1.8
// Build: 1803
// Date: 01-22-2007
...
$manager = "members";
$page = "Members Manager > Edit Member";
...
function check_dup_email_local($email, $member_id) {
	$email_test = "";
	$query = "SELECT email FROM members WHERE email='$email' AND member_id!='$member_id'";
...
if($submit != "") {
	...
	if($error_txt == "") {
		$email_test = check_dup_email_local($email, $member_id);
		if($email_test < 0) {
	...
	if($error_txt == "") {
		$queryBase = "UPDATE members SET email='$email', username='$email', ";
		
		if ( $password ) {
			$queryBase .= " password='".md5($password)."', ";
		}

		$queryBase .= " nickname='$nickname', first_name='$first_name', last_name='$last_name', date_of_birth='".date("Y-m-d H:i:s", strtotime($dob))."', bill_name='$bill_name', bill_address1='$bill_address1', bill_address2='$bill_address2', bill_city='$bill_city', bill_state='$bill_state', bill_zip='$bill_zip', bill_country='$bill_country', bill_phone='$bill_phone', ship_name='$ship_name', ship_address1='$ship_address1', ship_address2='$ship_address2', ship_city='$ship_city', ship_state='$ship_state', ship_zip='$ship_zip', ship_country='$ship_country', ship_phone='$ship_phone'";
		$query = $queryBase." WHERE member_id='$member_id'";

		$result = mysql_query($query) or die("INSERT Query failed : " . mysql_error().'<br /><br />'.$query);

		if( $email != $old_email || $password != "" ) {
		...
		while ($lineSites = mysql_fetch_array($resultSites, MYSQL_ASSOC)) {
			...
			if($member_email_test > 0) {//already exists so update with same info as referring site
				$updateQ = $queryBase." WHERE email='$email'";
...
$query = "SELECT email, password, nickname, first_name, last_name, date_of_birth, bill_name, bill_address1, bill_address2, bill_city, bill_state, bill_zip, bill_country, bill_phone, ship_name, ship_address1, ship_address2, ship_city, ship_state, ship_zip, ship_country, ship_phone FROM members WHERE member_id='$member_id'";
$result = mysql_query($query) or die("Query failed : " . mysql_error());
while ($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
	$email = $line["email"];
	...
	$bill_address1 = $line["bill_address1"];
	$bill_address2 = $line["bill_address2"];
	$bill_city = $line["bill_city"];
	$bill_state = $line["bill_state"];
	$bill_zip = $line["bill_zip"];
	$bill_country = $line["bill_country"];
	$bill_phone = $line["bill_phone"];
	$ship_name = $line["ship_name"];
	$ship_address1 = $line["ship_address1"];
	$ship_address2 = $line["ship_address2"];
	$ship_city = $line["ship_city"];
	$ship_state = $line["ship_state"];
	$ship_zip = $line["ship_zip"];
	$ship_country = $line["ship_country"];
	$ship_phone = $line["ship_phone"];
}
mysql_free_result($result);
?>
	...
	<script language="JavaScript">
		...
		$(document).ready(function(){
			$('#place_order').click(function(){
				...
				$(':submit[@name=submit]').attr('name', 'submit2');//have to rename the submit button or submit() function fails
				this.form.submit();
			});
		});
	</script>
...
<body topmargin="0" leftmargin="0" rightmargin="0" bottommargin="0" OnLoad="document.update_member.email.focus();">
<div align="center">
...
<tr><td align="left"><font size="2">Welcome to the Members Manager Edit Member page, where you edit a member of your website.</font></td></tr>

<tr><td>&nbsp;</td></tr>

<?php
echo '<tr><td><a href="members_admin2.php">&lt;&lt; return to Manage Members list</a></td></tr>';
//Error Messages
if($error_txt) {
	...
	if($submit != "") {
		echo '<tr><td><div class="error3">Changes Saved</div></td></tr>';
	}
}
?>

<tr><td align="left">
	<FORM name="update_member" Method="POST" ACTION="./members_admin2_edit.php" class="wmsform">
	<input type="hidden" name="member_id" value="<?php echo $member_id; ?>">
	<input type="hidden" name="old_email" value="<?php if($email != "") { echo $email; } ?>">
	...
	<fieldset>
		...
		<ol>
			...
			<li>
				...
				<INPUT type="text" id="password" name="password" size="20" value="" tabindex="2" />
			</li>
			<li>
			...
			<li>
    			...
    			<input type="text" id="dob" name="dob" tabindex="6" size="10" maxlength="10" value="<?php echo date("m/d/Y", strtotime($dob)); ?>">(mm/dd/yyy)
			</li>
		</ol>
	</fieldset>
	<fieldset>
	...
	<fieldset>
		...
		<ol>		
			...
			<li class="fm-button">
				<input type="button" id="order_history" name="order_history" value="View Order History" onClick="newWindow('./members_admin12.php?member_id=<?php echo $member_id; ?>','','650','250','resizable,scrollbars,status,toolbar')">
			</li>
			<li class="fm-button">
				<input type="button" id="place_order" name="place_order" value="Place Order">
			</li>
		</ol>
	</fieldset>
	</form>
