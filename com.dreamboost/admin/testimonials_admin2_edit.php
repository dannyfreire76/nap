<?php
// BME WMS
// Page: Testimonial Manager Edit Testimonials page
// Path/File: /admin/testimonials_admin2_edit.php
// Version: 1.8
// Build: 1804
// Date: 01-31-2007
...
$page = "Testimonials Manager > Edit Submitted Testimonials";
...
$result = mysql_query($query) or die("Query failed : " . mysql_error());
while ($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
	$product_name_question = $line["name"];
...
if($testimonial_id && $save) {
	//Check for Errors
	$error_txt = "";
	if($name_reqd_submit == "1" && $name == "") {
		$error_txt .= "Error, you removed the name. There needs to be a name.<br>";
	}
	...
	if($error_txt == "") {
		...
		if($status == "1") { //Approved
			$query = "SELECT content, subject, email FROM testimonial_emails WHERE email_id='2'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
			while ($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
			...
			$query = "SELECT content, subject, email FROM testimonial_emails WHERE email_id='3'";
			$result = mysql_query($query) or die("Query failed : " . mysql_error());
			while ($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
				$content = $line["content"];
				$subject = $line["subject"];
			...
			$email_str .= $content;
			$email_str .= "\n\n";
			$email_str .= "Your testimonial: " . $testimonial . "\n\n";
...
<tr><td align="left"><font size="2">Please edit the testimonial below. You can review and edit all the information submited. The key is to change the Status field from To Review to Approved or Rejected - Approved to launch live on the site.</font></td></tr>
...
<tr><td align="left">
	<FORM name="testimonial-edit" Method="POST" ACTION="./testimonials_admin2_edit.php" class="wmsform">
	...
	<fieldset>
		...
		<ol>
			...
			if($product_submit == "1") {
				...
				echo ">\n";
				echo "<label for=\"product\">$product_name_question";
				...
				$queryProds = "SELECT * FROM products WHERE prod_cat_id='1' OR prod_cat_id='11'";
				...
				while ($lineProds = mysql_fetch_array($resultProds, MYSQL_ASSOC)) {
					echo '<option value="'.$lineProds["prod_id"].'"';
					if($product == $lineProds["prod_id"]) { echo " SELECTED"; }
					echo '>'.$product_name.' '.$lineProds["name"].'</option>';
				}
				echo "</select>\n";
				echo "</li>\n";
			}
