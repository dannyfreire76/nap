...
while ($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
	...
	$prod_sku_data['weight_lbs'] = $line["prf_weight_lb"];
	$prod_sku_data['weight_ozs'] = $line["prf_weight_oz"];
...
for($i=0; $i< count($prod_sku2); $i++) {
		$query = "UPDATE product_skus SET weight_lbs='".$prod_sku2[$i]['weight_lbs']."', weight_ozs='".$prod_sku2[$i]['weight_ozs']."' WHERE sku='".$prod_sku2[$i]['sku']."'";
		$result = mysql_query($query) or die("Query failed : " . mysql_error());
}
