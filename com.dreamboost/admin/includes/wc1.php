...
/* This would NEVER work. city, state county, zip may not be set in rep_areas. They might be %.

		//*********** regardless of whether funds have been  paid, set commissions (funds_received determines if it actually gets paid, though)
		$queryReps = "SELECT reps.* from reps, reps_areas WHERE reps_areas.rep_id=reps.rep_id AND reps.rep_id IN ( SELECT rep_id FROM reps_areas WHERE reps_areas.state = '".$state."' AND '".$city."' LIKE reps_areas.city ";
...
*/
		...
		// Use OR's so we find someone if rep_areas fields use %

		$queryReps = "SELECT reps.* from reps, reps_areas 
					  WHERE reps_areas.rep_id = reps.rep_id
					  AND reps.status = '1'
					  AND (reps_areas.city LIKE '%$city%'
						   OR reps_areas.state = '$state'
						   OR reps_areas.county LIKE '%$county%'
						   OR reps_areas.zip = TRIM(LEADING '0' FROM '$zip'))
					  AND reps.rep_type_id IN(SELECT rep_type_id FROM retailer_rep_types WHERE retailer_id='$retailer_id')
					  GROUP BY reps.rep_id";
		
		//print "<!-- queryReps: ".$queryReps . "\n-->\n";
...
//print "retailer_status: $retailer_status\n";
//print "-->\n";
...
//print "-->\n";
				...
				if ( $thisComPct > 0 ) {
					...
					if ( trim($lineReps['rep_id']) == trim($_SESSION['rep_id']) ) {
						echo "When payment is received, you will earn a $thisComPct percent commission, for a total of \$$commEarned";
					}
				}
			}  
		}
