...
function state_build_all($state) {
	...
	echo "<option value=\"\">Select a State/Province</option>\n";
...
function state_build_active($state) {
	$query="SELECT code, name FROM states WHERE status='1' ORDER BY name";
	$result=mysql_query($query) or die("Query failed : " . mysql_error());
	while ($line=mysql_fetch_array($result, MYSQL_ASSOC)) { 
		$codes[] = $line["code"];
		$states[] = $line["name"];
...
function country_build_all($country) {
	$query="SELECT code, name FROM countries ORDER BY country_id";
	$result=mysql_query($query) or die("Query failed : " . mysql_error());
	while ($line=mysql_fetch_array($result, MYSQL_ASSOC)) { 
		$codes[] = $line["code"];
		$countries[] = $line["name"];
...
function country_build_active($country) {
	$query="SELECT code, name FROM countries WHERE status='1' ORDER BY name";
	$result=mysql_query($query) or die("Query failed : " . mysql_error());
	while ($line=mysql_fetch_array($result, MYSQL_ASSOC)) { 
		$codes[] = $line["code"];
		$countries[] = $line["name"];
