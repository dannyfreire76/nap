...
if ( $in_admin ) {//only for admin site
	...
	$tmp_startyr = date("Y", strtotime($rep_start_date));

	//DF: start_date should not be before this year; if it is, set it to first of year
	if ( $tmp_startyr < date("Y") ) {
		$rep_start_date = date('01/01/Y');
	}
...
$query .= " wholesale_receipts.*, retailer.*, wholesale_commissions.*, DATE_format(comm_paid, '%m/%d/%Y') as comm_paid_format, IF( comm_paid=0, 0, 1 ) AS comm_was_paid FROM retailer, wholesale_receipts ";
...
$query .= " AND wholesale_receipts.retailer_id IN ( ";//<IN
$query .= "SELECT retailer.retailer_id FROM retailer, reps_areas, retailer_type_link rtl, reps_industries ri ";
$query .= " WHERE ";
$query .= " rtl.retailer_type_id=ri.retailer_type_id";
$query .= " AND rtl.retailer_id = retailer.retailer_id";
$query .= " AND ri.rep_id = reps_areas.rep_id";

$query .= " AND retailer.country LIKE reps_areas.country ";

$query .= " AND ( ";

		$query .= "( retailer.country = 'US' ";

				$query .= " AND retailer.state LIKE reps_areas.state ";
				if ( $this_rep_id ) {
				...
				$query .= " AND retailer.city LIKE reps_areas.city ";
				$query .= " AND TRIM(LEADING '0' FROM retailer.zip) LIKE TRIM(LEADING '0' FROM reps_areas.zip) ";
				$query .= " AND retailer.city ";
				$query .= " IN (SELECT zip_codes.city FROM zip_codes, reps_areas";//<IN2
				$query .= " WHERE zip_codes.county LIKE reps_areas.county";
				$query .= "   AND zip_codes.state=reps_areas.state";
				$query .= "   AND zip_codes.city LIKE reps_areas.city";

		$query .= " ) ";
	
	$query .= " OR retailer.country!='US' ";
...
$query .= " ORDER BY comm_was_paid DESC, comm_paid ASC, ordered";
//echo $query;
